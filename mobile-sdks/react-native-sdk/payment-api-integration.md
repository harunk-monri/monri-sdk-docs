...
Monri has created a Java library for Android and Swift library for iOS, allowing you to easily submit payments from an
Android app. With our mobile library, we address PCI compliance by eliminating the need to send card data directly to
your server. Instead, our libraries send the card data directly to our servers, where we can convert them to tokens.
...
Integration example is available in SDKS's example and
on [this link](https://github.com/MonriPayments/react-native-monri-android-ios/blob/development/example/src/App.tsx)
